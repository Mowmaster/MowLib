...
public class WorkCardArea extends WorkCardBase implements ISelectableArea {
    ...
    // NOTE: The returned AABB is sufficient for cases that are dealing directly with the points covered by the area,
    // but needs to be modified by `.expandTowards(1.0D, 1.0D, 1.0D)` for any code that is using it as a bounding-box.
    public static Optional<AABB> getAABBIfDefinedAndInRange(ItemStack workCardStack, MowLibBaseBlockEntity baseBlockEntity, int rangeFromBlock) {
        BlockPos posOne = readBlockPosFromNBT(workCardStack, 1);
        BlockPos posTwo = readBlockPosFromNBT(workCardStack, 2);
        if (
            workCardStack.getItem() instanceof WorkCardArea workCard &&
    ...
    public static List<BlockPos> getPositionsInRangeOfUpgrade(ItemStack workCardStack, MowLibBaseBlockEntity baseBlockEntity, int rangeFromBlock) {
        return getAABBIfDefinedAndInRange(workCardStack, baseBlockEntity, rangeFromBlock)
            .map(area -> {
                ...
                if (minY < baseBlockEntity.getPos().getY()) {
    ...
    public static <T extends Entity> List<T> getEntitiesInRangeOfUpgrade(Level level, Class<T> entityType, ItemStack workCardStack, MowLibBaseBlockEntity baseBlockEntity, int rangeFromBlock) {
        return getAABBIfDefinedAndInRange(workCardStack, baseBlockEntity, rangeFromBlock)
